...
/**
 * 
 * Each instance of this class represents a matric with a given 
 * number of rows and
 * columns and double-valued elements
 * 
 * @invar | 0 <= getRowCount()
...
public class Matrix {
	 ...
	 * @post | result.length == getRowCount() * getColumnCount()
	 * @post | IntStream.range(0, getRowCount()).allMatch(i ->
	 * 		 |		IntStream.range(0, getRowCount()).allMatch(j -> 
	 * 		 |			result[i * getRowCount() + j] == getRows()[i][j]
	 * 		 |		)
	 * 		 |	)
	 * @creates | result
	 */
	 ...
	 * @inspect | elementsRowMajor
	 * @post | getRowCount() == rowCount
	 * @post | getColumnCount() == columnCount
	 * @post | Arrays.equals(getElementsRowMajor(), elementsRowMajor)
	 */
	public Matrix(int rowCount, int columnCount, double[] elementsRowMajor) { throw new RuntimeException( "Not yet implemented"); }
}
